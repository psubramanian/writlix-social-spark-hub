...
const LinkedInCredentialsForm = () => {
  ...
  const [loading, setLoading] = useState(false); // This is for submit loading
    ...
    if (isLoaded && user?.id) {
      fetchCredentials();
        ...
        toast({
            ...
            description: "Please log in to manage LinkedIn credentials.",
            variant: "destructive",
        });
    }
  }, [user?.id, isLoaded, toast, defaultRedirectUri]); // fetchCredentials is defined in scope
